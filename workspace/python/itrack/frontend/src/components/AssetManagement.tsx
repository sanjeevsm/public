...
export const AssetManagement: React.FC = () => {
  ...
  const assetCategories = ['Cash', 'Investments', 'Property', 'Valuables', 'Other'];
  ...
  const loadAssets = async () => {
    try {
      ...
      setAssets(data.filter(a => a.currency === activeCurrency));
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingAsset) {
        await transactionService.updateTransaction(editingAsset.id, dataToSubmit);
  ...
  const handleEdit = (asset: Transaction) => {
  ...
  const handleDelete = async (assetId: string) => {
    ...
    try {
      await transactionService.deleteTransaction(assetId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600">Track and manage your assets</p>
          </div>
        </div>
        <button
      ...
      <div className="card bg-gradient-to-r from-blue-500 to-cyan-600 text-white">
        <div className="flex justify-between items-center">
          <div>
            <p className="text-blue-100 text-sm">Total Assets ({activeCurrency})</p>
            <h2 className="text-4xl font-bold">{formatCurrency(totalAssets, activeCurrency)}</h2>
            <p className="text-blue-100 text-sm mt-1">{assets.length} assets</p>
          </div>
                      ...
                      <p className="text-sm text-gray-600" style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', flexWrap: 'wrap' }}>
                        ...
                        {hasEntity && asset.mode && (
                          <span style={{
                            ...
                            background: asset.mode === 'shared' ? 'var(--primary-soft)' : 'var(--surface-2)',
                            color: asset.mode === 'shared' ? 'var(--primary)' : 'var(--text-muted)',
                            border: `1px solid ${asset.mode === 'shared' ? 'var(--primary)' : 'var(--border)'}`,
                          }}>
                            {asset.mode === 'shared' ? <Users size={9} /> : <Lock size={9} />}
                            {asset.mode}
                          </span>
                        )}
                      </p>
