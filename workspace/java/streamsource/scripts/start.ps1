...
Set-Location $Root
$PidFile = Join-Path $Root ".streamsource.pid"
...
# Find JAR
$jar = Get-ChildItem "$Root\streamsource-app\target\streamsource-app-*.jar" -ErrorAction SilentlyContinue | Select-Object -First 1
if (-not $jar) {
...
$env:DB_URL           = if ($env:DB_URL)           { $env:DB_URL }           else { "jdbc:postgresql://localhost:5433/streamsource" }
$env:DB_USER          = if ($env:DB_USER)          { $env:DB_USER }          else { "streamsource" }
$env:DB_PASSWORD      = if ($env:DB_PASSWORD)      { $env:DB_PASSWORD }      else { "streamsource" }
...
Write-Info "Starting streamsource..."
$proc = Start-Process java `
    -ArgumentList "-XX:+UseContainerSupport", "-XX:MaxRAMPercentage=75.0", "-Djava.security.egd=file:/dev/./urandom", "-jar", $jar.FullName `
    -RedirectStandardOutput (Join-Path $Root "streamsource.log") `
    -RedirectStandardError  (Join-Path $Root "streamsource-err.log") `
    -PassThru -NoNewWindow

$proc.Id | Set-Content $PidFile
Write-Info "Application started (PID $($proc.Id)). Logs: streamsource.log"
...
if ($ready) {
    ...
    Write-Warn "Application did not respond within 60s. Check streamsource.log for errors."
}
