...
|---|---:|---|
| streamsource API | **8080** | REST + WebSocket |
...
```bash
git clone <repo-url>
cd workspace/java/streamsource
...
`setup.sh` checks Java/Maven versions, builds the project (`mvn clean package -DskipTests`), and copies `.env.example` → `.env`.
...
export DB_URL=jdbc:postgresql://localhost:5433/streamsource
export DB_USER=streamsource
export DB_PASSWORD=streamsource
...
$env:DB_URL        = "jdbc:postgresql://localhost:5433/streamsource"
$env:DB_USER       = "streamsource"
$env:DB_PASSWORD   = "streamsource"
...
```bash
java -jar streamsource-app/target/streamsource-app-*.jar
```
...
psql -U postgres -c "CREATE USER streamsource WITH PASSWORD 'streamsource';"
psql -U postgres -c "CREATE DATABASE streamsource OWNER streamsource;"
```
...
Then set `DB_URL=jdbc:postgresql://localhost:5432/streamsource` (note: 5432, not 5433, when PostgreSQL runs natively) and start the JAR as shown in Step 4.
...
| `DB_URL` | `jdbc:postgresql://localhost:5433/streamsource` | JDBC connection URL |
| `DB_USER` | `streamsource` | Database username |
| `DB_PASSWORD` | `streamsource` | Database password |
...
> All variables have safe defaults for local development. In production, override `JWT_SECRET`, `DB_PASSWORD`, and the external service credentials.
...
Application logs are written to `streamsource.log` in the project root.

```bash
tail -f streamsource.log
```
...
│
├── streamsource-core/                   # Shared domain — no Spring dependencies
│   └── src/main/java/com/streamsource/core/
│       ├── model/
...
│
├── streamsource-sources/                # Data source implementations
│   └── src/main/java/com/streamsource/sources/
...
│
├── streamsource-consumers/              # Kafka consumer group implementations
│   └── src/main/java/com/streamsource/consumers/
...
│
├── streamsource-api/                    # REST API, security, WebSocket
│   └── src/main/java/com/streamsource/api/
│       ├── controller/
...
│
├── streamsource-persistence/            # JPA, repositories, Flyway migrations
│   └── src/main/java/com/streamsource/persistence/
│       ├── entity/
...
│
└── streamsource-app/                    # Runnable entry point
    ├── Dockerfile                       # Multi-stage build (Maven + JRE Alpine)
    ├── src/main/java/com/streamsource/app/
    │   ├── StreamsourceApplication.java
    │   ├── config/
...
**Step 1 — Implement the interface** (one file in `streamsource-sources/`):

```java
// src/main/java/com/streamsource/sources/ForexSource.java
@Component
...
# Kafka topic list
docker exec -it streamsource-kafka-1 \
  kafka-topics --bootstrap-server localhost:9092 --list

# PostgreSQL table count
docker exec -it streamsource-postgres-1 \
  psql -U streamsource -c "SELECT COUNT(*) FROM market_events;"

# Redis ping
docker exec -it streamsource-redis-1 redis-cli PING
```
...
rm -f streamsource.log streamsource-err.log .streamsource.pid
mvn clean
./scripts/setup.sh
./scripts/start.sh
```
